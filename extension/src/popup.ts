...
async function setupWebcam(): Promise<boolean> {
  ...
  if (!videoElement || !tfStatusElement) {
    ...
    if (tfStatusElement)
      tfStatusElement.textContent = "Error: UI elements missing.";
    return false;
  }
  ...
  try {
    ...
    if (error instanceof Error) {
      if (
        ...
        errorMessage =
          "Webcam permission denied. Please grant access in browser settings.";
      } else if (
      ...
      chrome.tabs.sendMessage(
        ...
        (response?: { selectedText?: string }) => {
          if (chrome.runtime.lastError) {
            console.warn(
              "Error sending message (content script connection):",
          ...
          if (response && typeof response.selectedText === "string") {
            ...
            console.warn(
              "Received no response or invalid response format from content script."
            );
            const frontTextArea = document.getElementById(
              "card-front"
            ) as HTMLTextAreaElement | null;
...
function handleSaveCardClick() {
  ...
  if (frontText === "" || backText === "") {
    statusMessageElement.textContent =
      "Error: Both Front and Back fields are required.";
    statusMessageElement.style.color = "red";
...
async function initializeApp() {
  ...
  if (webcamReady && tfStatusElement) {
    ...
    if (handPoseModel) {
      ...
      console.error(
        "Model loading failed. Hand pose detection cannot proceed."
      );
    }
  } else {
